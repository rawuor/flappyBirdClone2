...
//  flappyBirdclone
//
//  Created by Awuor, Racquel on 4/21/15.
...
@interface MyScene() <SKPhysicsContactDelegate> {
    SKSpriteNode* _bird;
    SKColor* _skyColor; //add blue sky background
...
static NSInteger const kVerticalPipeGap = 100;//set the distance between the upper and the lower pipe = 100 points.
...
-(id)initWithSize:(CGSize)size {    
    if (self = [super initWithSize:size]) {
        ...
        //prevents bird from falling too fast. Simulates gravity
        
        _canRestart = NO;
        
        self.physicsWorld.gravity = CGVectorMake(0.0, -5.0); //This reduces gravity from -9.8m/s^2 to -5m/s^2
        ...
        SKTexture* birdTexture1 = [SKTexture textureWithImageNamed:@"Bird1"]; //Add bird1
        ...
        //ground animation: creates an illusion of a moving, or steady ground.
        ...
        //skyline animation: creates an illusion of a moving or a steady skyline
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    //This raises the bird when you click on it
    //Set velocity and the impulse
    if(_moving.speed > 0){
    _bird.physicsBody.velocity = CGVectorMake(0, 0);
    [_bird.physicsBody applyImpulse:CGVectorMake(0, 4)]; //Can change to too fast or too slow.
    }else if(_canRestart){
        [self resetScene]; //enable restarting when a collision happens. Call the resetScene method
    }
...
//this method flashes red on collision, so as to visualize the collision instances
-(void)didBeginContact:(SKPhysicsContact *)contact{
    ...
    //allow for the bird to fall on the ground during any collision, and then roll over
        _bird.physicsBody.collisionBitMask = worldCategory;
        [_bird runAction:[SKAction rotateByAngle:M_PI* _bird.position.y * 0.01 duration:_bird.position.y * 0.03] completion:^{ _bird.speed = 0; }];
                          
                          
    //Flash background if contact is detected
    [self removeActionForKey:@"flash"];
    [self runAction:[SKAction sequence:@[[SKAction repeatAction:[SKAction sequence:@[[SKAction runBlock:^{self.backgroundColor = [SKColor redColor];}], [SKAction waitForDuration:0.05], [SKAction runBlock:^{self.backgroundColor = _skyColor;}],[SKAction waitForDuration:0.05]]] count:4], [SKAction runBlock:^{_canRestart = YES;}]]] withKey:@"flash"];
        //[self runAction:[SKAction sequence:@[[SKAction repeatAction:[SKAction sequence:@[[SKAction runBlock:^{self.backgroundColor = [SKColor redColor];}], [SKAction waitForDuration:0.05], [SKAction runBlock:^{self.backgroundColor = _skyColor;}],[SKAction waitForDuration:0.05]]] count:4]]] withKey:@"flash"];
        }
    }
}

//implement the resetScene method
-(void) resetScene{
